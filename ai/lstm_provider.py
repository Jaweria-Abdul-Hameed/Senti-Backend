...
# keras/tensorflow/nltk are deliberately NOT in requirements.txt (see the comment
# there) -- they're huge (400MB+) and will blow Vercel's serverless size limit, so
# production won't have them installed at all. This import is wrapped instead of
# left at the top level so that missing keras doesn't crash the whole app on
# import -- EmotionClassifier.__init__ below raises a normal, catchable exception
# instead, which is what ai_setup.py's try/except around HybridProvider already
# expects and handles by falling back to GroqProvider only.
try:
    from keras.models import load_model
    from keras.preprocessing.sequence import pad_sequences
    from nltk.stem import WordNetLemmatizer

    _LSTM_DEPS_AVAILABLE = True
except ImportError:
    _LSTM_DEPS_AVAILABLE = False
...
_lemmatizer = WordNetLemmatizer() if _LSTM_DEPS_AVAILABLE else None
...
class EmotionClassifier:
    ...
    def __init__(self, model_path: str = "senti_emotion_lstm.keras", tokenizer_path: str = "senti_tokenizer.pkl"):
        if not _LSTM_DEPS_AVAILABLE:
            raise ImportError(
                "keras/nltk not installed -- HybridProvider/LocalLSTMProvider are "
                "unavailable in this environment. This is expected on Vercel (see "
                "requirements.txt). To use them locally, pip install keras tensorflow "
                "nltk (and run python -c \"import nltk; nltk.download('wordnet')\")."
            )
